...
function TeamView() {
	...
	const productLeaderUserName = users.find(
		(users) => users.name === 'Jane Doe'
	);
	...
	const handleSaveClick = async () => {
		...
		const updatedUser = await updateUsersById(user._id, formData);
		setUser(updatedUser);
	};
	...
	const handleInputChange = (e) => {
		if (e.target.name === 'socialConnections') {
			...
			<div className="flex justify-center">
				<div>
						...
						<div>
							...
							<div className="flex justify-center mr-[590px] relative z-[12px]">
								{user && user.profilePicture && (
									<img
										...
										alt="user2"
										className="w-[90px] h-[90px] absolute top-[-30px] ml-[40px] rounded-full"
									/>
								)}
							</div>
							<div className="text-xs ml-[100px] relative top-[14px]">
								<div className="absolute bottom-[-145px]">
									...
									{/* <p>Skills: </p> */}
									<p className="">
										Social Networks: {isEditing ? <></> : socialNetworks}
									</p>
								</div>
							</div>
						</div>
					...
					<div className="absolute bottom-1">
						<div
							className="ml-[322px] w-[308px] h-[328px] rounded-[20px] border-[2px] border-black flex justify-start"
						...
						>
							...
							<img
								src="./src/assets/logo_flag.png"
								alt="logo-flag"
								className="ml-[27px] w-[68px] h-[325px]  flex justify-start items-start mr-[740px]"
							/>
							{/* First developer */}
							<div className="absolute top-[50px] left-[340px] flex flex-col items-start ">
								<img
									src={productLeaderUserName?.profilePicture}
									alt="user2"
								...
								<div className="flex">
									<div className="absolute top-2 left-[100px]">
										{productLeaderUserName && (
											<p key={productLeaderUserName.id}>
												{' '}
												{productLeaderUserName.name}{' '}
											</p>
										)}
										{/* <p key={productLeaderRole.id}>{productLeaderRole.name}</p> */}
										...
										<p className="text-xs">
											{' '}
											{productLeaderUserName?.socialConnections}{' '}
										</p>
									</div>
									<img
										src="./src/assets/user5.png"
										alt="user2"
										className="w-[89px] h-[89px] absolute bottom-[-125px]"
									/>
					...
					<div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex justify-between ml-[296px]">
						<div className="mb-1 ml-10 w-[499px] h-[328px] rounded-[20px] border-[2px] border-black">
							<h1 className="ml-[12px] mt-2 font-bold text-3xl">
								{' '}
								{productDeveloperRole && (
									<p key={productDeveloperRole.id}>
										{productDeveloperRole.name}
									</p>
								)}
							</h1>
							...
							<div className="text-xs flex justify-start flex-col">
								...
								<div className="absolute top-[72px] ml-[90px]">
									{productDeveloperName && (
										<p key={productDeveloperName.id}>
											{productDeveloperName.name}{' '}
										</p>
									)}
							...
							<div className="text-xs flex justify-start flex-col">
								<img
									...
									alt="user2"
									className="w-[49px] h-[49px] ml-2 m-6"
								/>
							...
							<img
								...
								alt="user2"
								className="w-[49px] h-[49px] ml-2 absolute bottom-[125px]"
							/>
							<div className="text-xs absolute bottom-[-2px] right-[350px] m-6"></div>
					...
					<div className="absolute top-2 right-2">
						...
						<div className="w-[816px] h-[172px] rounded-[20px] border-[2px] border-black flex justify-end items-start">
							...
							<img
								src="./src/assets/logo_flag.png"
								alt="logo-flag"
								className="h-[169px] flex justify-start items-start mr-[718px] sticky"
							/>

							<img
								src="https://images.unsplash.com/photo-1521119989659-a83eee488004?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTB8fHBvcnRyYWl0fGVufDB8fDB8fHww"
								alt="user2"
								className="m-[74px] w-[89px] h-[89px] absolute left-[-60px] top-[-30px]"
							/>
...
{productOwnerRole && (
      <p key={productOwnerRole.id}>{productOwnerRole.name}</p>
								...
								{productOwnerName && (
									<p key={productOwnerName.id}>
										{productOwnerName.socialConnections}
									</p>
								)}
								<p>Social Networks:</p>
								<div
									className="flex flex-col absolute right-[320px] top-[4px] "
									style={{ width: '150px' }}
								>
									{' '}
